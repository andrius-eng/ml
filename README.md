...
One-time setup for trusted Tailscale HTTPS:
...
- HTTPS certificates must be enabled in the Tailscale admin DNS settings.
- The `desktop-nnutaj7` node must stay logged in to Tailscale because the
  renewal job shells into the host `tailscale cert` command.
