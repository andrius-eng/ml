...
# distilgpt2 (82M params) remains small enough for CPU-only LoRA runs.
...
with DAG(
    ...
    train_lora = BashOperator(
        ...
        bash_command=(
            ...
            f'{project_python_command(str(TRAIN_SCRIPT), "--train-jsonl", str(LLM_OUTPUT_DIR / "sft_train.jsonl"), "--eval-jsonl", str(LLM_OUTPUT_DIR / "sft_eval.jsonl"), "--base-model", BASE_MODEL, "--output-dir", str(LLM_OUTPUT_DIR / "lora-adapter"), "--max-length", "256", "--epochs", "3", "--learning-rate", "5e-4")}'
        ),
